...
1. Run `./start.sh` — the server starts and your browser opens automatically
2. First launch: scan the QR code with WhatsApp on your phone (one-time setup)
3. Search a contact, type a message, pick a send time
...
- **Contact search** — debounced autocomplete from your saved WhatsApp contacts
...
├── start.sh            # Entry point — installs deps, runs caffeinate + node
└── .gitignore
```
